...
def load_audio_from_video(video_path: str, sampling_rate: int = 16000):
    """
    ...
    try:
        ...
        duration_in_seconds = min(video_duration, 300)
        
        num_samples = int(duration_in_seconds * sampling_rate)
    ...
    if duration > 300:
        print(f"Audio duration {duration:.2f}s > 300s, performing segmented sampling...")
        
        num_segments = 300
        samples_per_second = sr
        
        total_samples = len(audio)
        segment_size_samples = total_samples // num_segments
        
        audio_chunks = []
        for i in range(num_segments):
            segment_start = i * segment_size_samples
            segment_mid = segment_start + (segment_size_samples // 2)
            
            extract_start = segment_mid - (samples_per_second // 2)
            extract_end = extract_start + samples_per_second
        ...
        return new_audio, new_duration
        
    if len(audio) < sr * 2:
        padding = np.zeros(sr * 2 - len(audio), dtype=audio.dtype)
        audio = np.concatenate((audio, padding))

    duration = math.ceil(len(audio) / sr)

    if duration > 1 and duration % 2 != 0:
        samples_to_keep = (duration - 1) * sr
